...
let wordsList = function (str, subStr) {
  ...
  arr.forEach((arrItem) => r.add(arrItem));
...
const getDateAgo = function (date, days) {
  ...
  dayNow.setDate(dayNow.getDate() - days);

  let d = dayNow.getDate();
  let m = dayNow.getMonth();
  m++;
  let y = dayNow.getFullYear();
  let res = 0;
  if (m < 9 ? (res = `${d}.0${m}.${y}`) : (res = `${d}.${m}.${y}`));
